...
class Generator(nn.Module):
    def __init__(self, scale, num_rrdb_blocks=16):
...
def _esrgan(arch, num_residual_block, pretrained, progress, scale) -> Generator:
    model = Generator(scale, num_residual_block)
    if pretrained:
...
def esrgan16(scale, pretrained: bool = False, progress: bool = True) -> Generator:
...
def esrgan23(scale, pretrained: bool = False, progress: bool = True) -> Generator:
    r"""GAN model architecture from the
    `"One weird trick..." <https://arxiv.org/abs/1809.00219>`_ paper.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
